...
#if defined(__APPLE__)
    // Grok Build+ patch: never let inherited process environment authorize
    // whisper.cpp to fork and attach LLDB. Native in-process symbols are the
    // only admitted macOS failure path.
